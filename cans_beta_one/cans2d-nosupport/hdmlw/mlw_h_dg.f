c======================================================================|
      subroutine mlw_h_dg(ro,pr,vx,vy,dt,qav,gm
     &           ,gx,gxm,gy,gym
...
c        * hydrodynamics
c        * Cylindrical coordinate, transverse-symmetry
c
c INPUTS & OUTPUTS
...
c        ep=pr(i,j)*gm/(gm-1.)+0.5*ro(i,j)*vv
         ep=pr(i,j)+ee(i,j)
         fx(i,j)= ep*vx(i,j)
         fy(i,j)= ep*vy(i,j)/x(i)
         ss(i,j)= -fx(i,j)/x(i)
     &            +ro(i,j)*(vx(i,j)*gx(i,j)+vy(i,j)*gy(i,j))
      enddo
      enddo
...
c---  x-momentum ---
      ...
      do i=1,ix
         ...
         fy(i,j)= ro(i,j)*vx(i,j)*vy(i,j)/x(i)
         ss(i,j)= -ro(i,j)/x(i)*(vx(i,j)**2-vy(i,j)**2)
     &             +ro(i,j)*gx(i,j)
      enddo
      enddo
...
c---  y-momentum ---
      ...
      do i=1,ix
         ...
         fy(i,j)= ro(i,j)*vy(i,j)**2+pr(i,j)/x(i)
         ss(i,j)= -2.d0*ro(i,j)*vx(i,j)*vy(i,j)/x(i)
     &             +ro(i,j)*gy(i,j)
      enddo
      enddo
...
c---  density  ---
      ...
      do i=1,ix-1
         ...
         fy(i,j)= roh(i,j)*vyh(i,j)/xm(i)
         ss(i,j)= -fx(i,j)/xm(i)
      enddo
      enddo
      call mlwfull(dro ,dt,fx,dxi,ux0,ux1,ix,fy,dyi,uy0,uy1,jx)
      call mlwsrcf(dro,dt,ss,ux0,ux1,ix,uy0,uy1,jx)

c---  energy     ---
      do j=1,jx-1
      do i=1,ix-1
c        vv=vxh(i,j)**2+vyh(i,j)**2
c        eph=prh(i,j)*gm/(gm-1.)+0.5*roh(i,j)*vv
         eph=prh(i,j)+eeh(i,j)
         fx(i,j)= eph*vxh(i,j)
         fy(i,j)= eph*vyh(i,j)/xm(i)
         ss(i,j)= -fx(i,j)/xm(i)
     &    +roh(i,j)*(vxh(i,j)*gxm(i,j)+vyh(i,j)*gym(i,j))
      enddo
      enddo
...
c---  x-momentum ---
      ...
      do i=1,ix-1
         ...
         fy(i,j)= roh(i,j)*vxh(i,j)*vyh(i,j)/xm(i)
         ss(i,j)= -roh(i,j)/xm(i)*(vxh(i,j)**2-vyh(i,j)**2)
     &             +roh(i,j)*gxm(i,j)
      enddo
      enddo
...
c---  y-momentum ---
      ...
      do i=1,ix-1
         ...
         fy(i,j)= roh(i,j)*vyh(i,j)**2+prh(i,j)/xm(i)
         ss(i,j)= -2.d0*roh(i,j)*vxh(i,j)*vyh(i,j)/xm(i)
     &             +roh(i,j)*gym(i,j)
      enddo
      enddo
