...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,pr,bx,by,gm,rkap0,margin,x,ix,y,jx,mf_params
     &           ,igx,jgx,ipe,jpe)
      mper=(/0,0/)
...
c     data output
      ...
      write(mf_ro) ro
      mf_bx=14
      call dacdef2d(mf_bx,'bx.dac',6,ix,jx)
      write(mf_bx) bx
      mf_by=15
      call dacdef2d(mf_by,'by.dac',6,ix,jx)
      write(mf_by) by
      mf_az=16
      call dacdef2d(mf_az,'az.dac',6,ix,jx)
      write(mf_az) az
...
c     solve conduction equation
         ...
         call p_cndsor_m(ro,pr,mi,err,dt,gm,rkap0,bx,by
