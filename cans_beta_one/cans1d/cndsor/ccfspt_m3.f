c======================================================================|
      subroutine ccfspt_m3(cmat,src,rkap0,gm,dt,te,ro,bxm,by,bz
     &                                            ,dx,dxm,ix)
c======================================================================|
c
c NAME  ccfspt_m3
...
c        te00=0.5*(te(i)+te(i+1))
         ...
         by00=0.5*(by(i)+by(i+1))
         bz00=0.5*(bz(i)+bz(i+1))
         ebb=bxm(i)**2/(by00**2+bz00**2+bxm(i)**2)
         axx(i)=rkap0*sqrt(min(te00,telim))**5*ebb
      enddo
      do i=1,ix
         a0t(i)=ro(i)/(gm-1.)
      enddo
