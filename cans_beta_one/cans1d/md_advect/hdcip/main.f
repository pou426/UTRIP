...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,vx,vxm,margin,x,ix,mf_params)

      call grdrdy(dx,xm,dxm,x,ix)
      call ciprdy_a(ro,rodx,dx,ix)
      call cipbnd(margin,rodx,ix)

      call bnd(margin,ro,ix)
c----------------------------------------------------------------------|
c  read-data
...
c     data output
      ...
      close(mf_x)
      mf_vx=12
      call dacdef1d(mf_vx,'vx.dac',6,ix)
      write(mf_vx) vx
      close(mf_vx)
      mf_vxm=13
      call dacdef1d(mf_vxm,'vxm.dac',6,ix)
      write(mf_vxm) vxm
      close(mf_vxm)
...
c     obtain time spacing
         ...
         safety=1.0d0
         dtmin=1.d-10
         call cfl_a(dt,safety,dtmin,merr,vx,dx,ix)
...
c     solve hydrodynamic equations
         ...
         call cip_a(ro,rodx,dt,vx,vxm,dx,dxm,ix)
         call cipbnd(margin,rodx,ix)
