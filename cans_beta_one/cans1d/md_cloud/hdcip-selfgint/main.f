...
c======================================================================|
      ...
      dimension vxm(ix),rodx(ix),vxdxm(ix)
...
c  file open
      ...
      mf_gx=32
      call dacdef1s(mf_gx,'gx.dac',6,ix)
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,vx,cs2,g0,margin,x,ix
     &     ,mf_params)

      call grdrdy(dx,xm,dxm,x,ix)

      call bnd(margin,ro,vx,ix)

      call ciprdy_ht(vxm,rodx,vxdxm,ro,vx,dx,dxm,ix)
      call cipbnd(margin,ro,vxm,rodx,vxdxm,ix)

         xmc=0.d0
         call selfgint(gx,gxm,g0,ro,xmc,x,dx,dxm,ix)
c     floor=1.d-9
...
c  read-data
      ...
      mfi_gx=73
      call dacopnr1s(mfi_gx,'in/gx.dac',mtype,ix0,nx0)
      do n=1,ndi
...
c     data output
      ...
      call dacputparamd(mf_params,'g0',g0)
...
c     obtain time spacing
         ...
         safety=0.8d0
         dtmin=1.d-10
         call cfl_ht(dt,safety,dtmin,merr,cs2,vx,dx,ix)
...
c     solve hydrodynamic equations
         ...
         cvis=0.75d0
         call cip_ht_g(ro,vx,vxm,rodx,vxdxm,dt,cvis,cs2,gxm,dx,dxm,ix)
