...
c----------------------------------------------------------------------|
      do i=1,ix
        ...
        gxm(i)=-1/gm*rstar**2/(x(i)+0.5*dxm(i))**2
      enddo
c----------------------------------------------------------------------|
...
c       te = 1+(teism-1)*(1+tanh((x(i)-rhs)/0.5d0))/2
        te = (teism+(1-teism)*(1-(x(i)-rhs)/(x(ix-margin)-rhs)))
...
c----------------------------------------------------------------------|
      pi = acos(-1.0d0)
      betai=0.1
      bx0=sqrt(betai*pr(izero)*8*pi)
      do i=1,ix
        vy(i)=0.
        bx(i)=bx0*x(izero)**2/x(i)**2
        bxm(i)=bx0*x(izero)**2/(x(i)+dxm(i)/2)**2
        by(i)=0
      enddo
c----------------------------------------------------------------------|
