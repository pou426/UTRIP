c======================================================================|
      subroutine mlw_mh_c(rof,prf,ehf,vxf,vyf,vzf,bxf,byf,bzf
     &  ,exf,eyf,ezf,xf
...
c---  density ---
      ...
      call fx2c(fy,ss,fx,xf,ix,jx,kx,mfdim)
      if (mstage.eq.1) then
        call mlwh(roh,ro2
...
c---  x-momentum ---
     ...
     &      ,ix,jx,kx,mfdim3(:,1))
      ...
      do i=1,ix
         ...
         ss(i,j,k)= -rof(i,j,k)/xf(i)*(vxf(i,j,k)**2-vyf(i,j,k)**2)
     &                   +(bxf(i,j,k)**2-byf(i,j,k)**2)/mu/xf(i)
      enddo
      enddo
      enddo
      if (mstage.eq.1) then
...
c---  y-momentum ---
     ...
     &      ,ix,jx,kx,mfdim3(:,2))
      ...
      do i=1,ix
         fy(i,j,k)= fy(i,j,k)/xf(i)
         ss(i,j,k)= -2.d0*rof(i,j,k)/xf(i)*vxf(i,j,k)*vyf(i,j,k)
     &                   +2.d0/mu/xf(i)*bxf(i,j,k)*byf(i,j,k)
      enddo
      enddo
      enddo
      if (mstage.eq.1) then
