...
c======================================================================|
      ...
      parameter (margin=1)
      parameter (ix=32+2*margin,jx=32+2*margin,kx=32+2*margin)
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,pr,bx,by,bz,gm,rkap0,margin,x,ix,y,jx,z,kx
     &   ,mf_params)
...
c     data output
      ...
      mf_ro=14
      call dacdef3d(mf_ro,'ro.dac',6,ix,jx,kx)
      write(mf_ro) ro
      mf_bx=15
      call dacdef3d(mf_bx,'bx.dac',6,ix,jx,kx)
      write(mf_bx) bx
      mf_by=16
      call dacdef3d(mf_by,'by.dac',6,ix,jx,kx)
      write(mf_by) by
      mf_bz=17
      call dacdef3d(mf_bz,'bz.dac',6,ix,jx,kx)
      write(mf_bz) bz
...
c     solve conduction equation
         ...
         call cndbicg_m(ro,pr,mi,err,dt,gm,rkap0,bx,by,bz
