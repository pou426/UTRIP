c======================================================================|
      subroutine mlw_m_t(ro,pr,vx,vy,vz,bx,by,bz,dt,qav,gm
     &                 ,omega0,qt0,x,xm,dx,dxm,ix,dy,dym,jx,dz,dzm,kx)
c======================================================================|
c
c NAME  mlw_m_t
c
...
c---  energy ---
      ...
      do i=1,ix       
         ...
         ss(i,j,k)= 2.0d0*qt0*omega0**2*x(i)*ro(i,j,k)*vx(i,j,k)
      enddo
      enddo
      enddo
...
c---  x-momentum ---
     ...
     &          -pi4i*bx(i,j,k)*bz(i,j,k)
         ss(i,j,k)= 2.0d0*qt0*omega0**2*x(i)*ro(i,j,k)
     &             +2.0d0*omega0*ro(i,j,k)*vy(i,j,k)
      enddo
      enddo
      enddo
...
c---  y-momentum ---
     ...
     &          -pi4i*by(i,j,k)*bz(i,j,k)
         ss(i,j,k)= -2.0d0*omega0*ro(i,j,k)*vx(i,j,k)
      enddo
      enddo
      enddo
...
c---  energy     ---
      ...
      do i=1,ix-1
         ...
         ss(i,j,k)= 2.0d0*qt0*omega0**2*xm(i)*roh(i,j,k)*vxh(i,j,k)
      enddo
      enddo
      enddo
      call mlwfull(dee,dt
...
c---  x-momentum ---
     ...
     &          -pi4i*bxh(i,j,k)*bzh(i,j,k)
         ss(i,j,k)= 2.0d0*qt0*omega0**2*xm(i)*roh(i,j,k)
     &           +2.0d0*omega0*roh(i,j,k)*vyh(i,j,k)
      enddo
      enddo
      enddo
      call mlwfull(drx,dt
...
c---  y-momentum ---
     ...
     &          -pi4i*byh(i,j,k)*bzh(i,j,k)
         ss(i,j,k)= -2.0d0*omega0*roh(i,j,k)*vxh(i,j,k)
      enddo
      enddo
      enddo
      call mlwfull(dry,dt
