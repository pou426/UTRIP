c======================================================================|
      subroutine mlw_m_sge(ro,pr,vx,vy,vz,bx,by,bz,dt,qav,gm
     &     ,gx,gxm,gy,gym,gz,gzm,et,etm
...
c----------------------------------------------------------------------|
     ...
     &         ,dbxsc(ix,jx,kx),dbysc(ix,jx,kx),dbzsc(ix,jx,kx)
      ...
      dimension cx(ix,jx,kx),cxh(ix,jx,kx)
      dimension cy(ix,jx,kx),cyh(ix,jx,kx)
      dimension cz(ix,jx,kx),czh(ix,jx,kx)
...
c----------------------------------------------------------------------|
      ...
      call bbtocx_s(cx,by,bz,dy,dz,x,y,ix,jx,kx)
      call bbtocy_s(cy,bz,bx,dz,dx,x,y,ix,jx,kx)
      call bbtocz_s(cz,bx,by,dx,dy,x,ix,jx,kx)
...
c---  energy ---
     ...
     &               +vz(i,j,k)*gz(i,j,k))*x(i)**2*siny(j)
      enddo
      enddo
      enddo
      call mlwhalf(eesc,eesch,deesc,dt
     &             ,fx,dxi,dxim,ix,fy,dyi,dyim,jx,fz,dzi,dzim,kx)
      call mlwsrch(eesch,deesc,dt,ss,ix,jx,kx)
...
c----------------------------------------------------------------------|
      ...
      call bbtocx_s(cxh,byh,bzh,dym,dzm,xm,ym,ix,jx,kx)
      call bbtocy_s(cyh,bzh,bxh,dzm,dxm,xm,ym,ix,jx,kx)
      call bbtocz_s(czh,bxh,byh,dxm,dym,xm,ix,jx,kx)
...
c---  energy     ---
     ...
     &                +vzh(i,j,k)*gzm(i,j,k))*xm(i)**2*sinym(j)
      enddo
      enddo
      enddo
      call mlwfull(deesc,dt
     &       ,fx,dxi,ux0,ux1,ix,fy,dyi,uy0,uy1,jx,fz,dzi,uz0,uz1,kx)
      call mlwsrcf(deesc,dt,ss,ux0,ux1,ix,uy0,uy1,jx,uz0,uz1,kx)
