c======================================================================|
      subroutine cipbdb3sym(bxm,bym,bzm
...
c----------------------------------------------------------------------|
      ...
      double precision bzm
      dimension bxm(ix,jx),bym(ix,jx),bzm(ix,jx)
c----------------------------------------------------------------------|
...
c       -- Bt(bnd_out) = +Bt(bnd_in)
        if (msign_t.eq.+1) then 
          ...
          call bdsppx(mbnd,margint,bzm,ix,jx)
c       -- Bt(bnd_out) = -Bt(bnd_in)
        else
          call bdspnx(mbnd,margint,bym,ix,jx)
          call bdspnx(mbnd,margint,bzm,ix,jx)
        endif
...
c       -- Bt(bnd_out) = +Bt(bnd_in)
        if (msign_t.eq.+1) then 
          ...
          call bdsppy(mbnd,margint,bzm,ix,jx)
c       -- Bt(bnd_out) = -Bt(bnd_in)
        else
          call bdspny(mbnd,margint,bxm,ix,jx)
          call bdspny(mbnd,margint,bzm,ix,jx)
        endif
