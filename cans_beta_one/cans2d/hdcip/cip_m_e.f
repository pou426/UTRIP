c======================================================================|
      subroutine cip_m_e(ro,pr,vx,vxm,vy,vym,te
     &             ,rodx,rody,tedx,tedy,vxdxm,vxdym,vydxm,vydym
     &             ,bx,bxm,by,bym,az
     &             ,dt,cvis,gm,etm,dx,dxm,ix,dy,dym,jx)
c======================================================================|
c
c NAME  cip_m_e
c
...
c--- ### important ### Use (n+1)-step values for vx & vy !
      ...
      call cipb2cz(czc,bxm,bym,dxm,dym,ix,jx)
      call ctranspt_e(bxmh,bymh,vxc,vyc,bxc,byc,etm,czc,dt,dx,dy,ix,jx)
...
c----------------------------------------------------------------------|
     ...
     &           +etm(i,j)*czc(i,j)
      ...
      do j=2,jx
      do i=2,ix
         ez = (ezc(i-1,j)+ezc(i,j)+ezc(i-1,j-1)+ezc(i,j-1))/4
         az(i,j)=az(i,j)+dt*(-ez)
      enddo
      enddo
