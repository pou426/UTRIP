c======================================================================|
      subroutine mocz(vznx,vzny,bznx,bzny,ro,vxm,bxm,vym,bym,vz,bz
     &      ,dt,dxm,dym,ix,jx)
c======================================================================|
c
c NAME  moclag
...
c----------------------------------------------------------------------|
      ...
      dimension ro(ix,jx),vxm(ix,jx),bxm(ix,jx),vym(ix,jx),bym(ix,jx)
...
c- Characteristic velocity
      ...
      do i=1,ix-1
        ...
        car(i,j)= vxm(i,j)+bxm(i,j)/sqrt(4.*pi*ronx)
        cal(i,j)= vxm(i,j)-bxm(i,j)/sqrt(4.*pi*ronx)
      enddo
      enddo
...
c- MOC
      ...
      do i=1,ix-1
        ...
        vznx(i,j)=(vr(i,j)*ror4+vl(i,j)*rol4-br(i,j)+bl(i,j))
     &          /(ror4+rol4)
...
c- Characteristic velocity
      ...
      do i=1,ix
        ...
        car(i,j)= vym(i,j)+bym(i,j)/sqrt(4.*pi*rony)
        cal(i,j)= vym(i,j)-bym(i,j)/sqrt(4.*pi*rony)
      enddo
      enddo
...
c- MOC
      ...
      do i=1,ix
        ...
        vzny(i,j)=(vr(i,j)*ror4+vl(i,j)*rol4-br(i,j)+bl(i,j))
     &          /(ror4+rol4)
