c======================================================================|
      subroutine ccfspt_m3s(cmat,src,rkap0,gm,dt,te,ro,bx,by,bz
...
c        te00=0.5*(te(i,j)+te(i+1,j))
         ...
         tmp=rkap0*sqrt(min(te00,telim))**5*bx00/bf2*xm(i)**2*sin(y(j))
         axx(i,j)=tmp*bx00
         axy(i,j)=tmp*by00
      enddo
      enddo
...
c        te00=0.5*(te(i,j)+te(i,j+1))
         ...
         bf2=(sqrt(bx00**2+by00**2+bz00**2)+sft)**2
         tmp=rkap0*sqrt(min(te00,telim))**5*by00/bf2*x(i)*sin(ym(j))
         ayy(i,j)=tmp*by00
         ayx(i,j)=tmp*bx00
      enddo
      enddo
