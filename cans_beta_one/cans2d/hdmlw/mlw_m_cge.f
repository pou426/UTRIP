c======================================================================|
      subroutine mlw_m_cge(ro,pr,vx,vz,bx,bz,ay,dt,qav,gm
...
c---  x-momentum ---
     ...
     &              +pi4i*(-bx(i,j)**2))/x(i)+ro(i,j)*gx(i,j)
      enddo
      enddo
...
c---  x-momentum ---
     ...
     &              +pi4i*(-bxh(i,j)**2))/xm(i)+roh(i,j)*gxm(i,j)
      enddo
      enddo
...
c---  z-momentum ---
     ...
     &          +pi8i*(bxh(i,j)**2-bzh(i,j)**2)+roh(i,j)*gzm(i,j)
         ss(i,j)= -fx(i,j)/xm(i)
      enddo
      enddo
