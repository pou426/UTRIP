c======================================================================|
      subroutine mlw_ht_s(ro,vx,vy,dt,qav,cs2
...
c----------------------------------------------------------------------|
      ...
      dimension pr(ix,jx),rx(ix,jx),ry(ix,jx)
      dimension roh(ix,jx),vxh(ix,jx),vyh(ix,jx)
      dimension prh(ix,jx)
...
c---  x-momentum ---
      ...
      do i=1,ix
         ...
         fx(i,j)= fx(i,j)*x(i)**2*siny(j)
         fy(i,j)= fy(i,j)*x(i)*siny(j)
         ss(i,j)= (ro(i,j)*vy(i,j)**2+2*pr(i,j))*x(i)*siny(j)
      enddo
      enddo
...
c---  y-momentum ---
      ...
      do i=1,ix
         ...
         ss(i,j)= (ro(i,j)*vx(i,j)*vy(i,j))*(-2)*siny(j)
     &          + pr(i,j)*cos(y(j))
      enddo
      enddo
...
c----------------------------------------------------------------------|
      do j=1,jx-1
      do i=1,ix-1
         roh(i,j)   = rosch(i,j)/xm(i)**2/siny(j)
         vxh(i,j)   = rxsch(i,j)/rosch(i,j)
         vyh(i,j)   = rysch(i,j)/rosch(i,j)*xm(i)
         prh(i,j)   = roh(i,j)*cs2
      enddo
      enddo
c----------------------------------------------------------------------|
...
c---  x-momentum ---
      ...
      do i=1,ix-1
         ...
         fx(i,j)= fx(i,j)*xm(i)**2*sinym(j)
         fy(i,j)= fy(i,j)*xm(i)*sinym(j)
         ss(i,j)= (roh(i,j)*vyh(i,j)**2+2*prh(i,j))*xm(i)*siny(j)
      enddo
      enddo
...
c---  y-momentum ---
      ...
      do i=1,ix-1
         ...
         ss(i,j)= (roh(i,j)*vxh(i,j)*vyh(i,j))*(-2)*sinym(j)
     &          + prh(i,j)*cos(ym(j))
      enddo
      enddo
...
c----------------------------------------------------------------------|
      ...
      do i=2,ix-1  
         ...
         vx(i,j) = rxsc(i,j)/rosc(i,j)
         vy(i,j) = rysc(i,j)/rosc(i,j)*x(i)
      enddo
      enddo
