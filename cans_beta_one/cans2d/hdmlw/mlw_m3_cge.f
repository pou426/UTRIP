c======================================================================|
      subroutine mlw_m3_cge(ro,pr,vx,vy,vz,bx,by,bz,ay,dt,qav,gm
     &                   ,gx,gxm,gz,gzm,et,etm,x,xm,dx,dxm,ix,dz,dzm,jx)
c======================================================================|
c
c NAME  mlw_m3_cge
c
...
c----------------------------------------------------------------------|
      ...
      call bbtocx_c(cx,by,dz,ix,jx)
      call bbtocy_c(cy,bz,bx,dz,dx,ix,jx)
      call bbtocz_c(cz,by,dx,x,ix,jx)
...
c---  energy ---
      ...
      do i=1,ix       
         ...
         fz(i,j)= ep*vz(i,j) +(by(i,j)*ex(i,j)-bx(i,j)*ey(i,j))*pi4i
         ss(i,j)= -fx(i,j)/x(i)
     &            +ro(i,j)*(vx(i,j)*gx(i,j)+vz(i,j)*gz(i,j))
      enddo
      enddo
...
c---  x-momentum ---
     ...
     &          +pi4i*(by(i,j)**2-bx(i,j)**2))/x(i)+ro(i,j)*gx(i,j)
      enddo
      enddo
...
c----------------------------------------------------------------------|
      ...
      call bbtocx_c(cxh,byh,dzm,ix,jx)
      call bbtocy_c(cyh,bzh,bxh,dzm,dxm,ix,jx)
      call bbtocz_c(czh,byh,dxm,xm,ix,jx)
...
c---  energy     ---
     ...
     &            +(bzh(i,j)*eyh(i,j)-byh(i,j)*ezh(i,j))*pi4i
         fz(i,j)= ep*vzh(i,j)
     &            +(byh(i,j)*exh(i,j)-bxh(i,j)*eyh(i,j))*pi4i
         ss(i,j)= -fx(i,j)/xm(i)
     &            +roh(i,j)*(vxh(i,j)*gxm(i,j)+vzh(i,j)*gzm(i,j))
      enddo
      enddo
