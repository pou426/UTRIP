      subroutine roeflux_m3(fro,fee,frx,fry,frz,fby,fbz,gm
     &              ,row,prw,vxw,vyw,vzw,bxw,byw,bzw,ix,jx)

      implicit double precision (a-h,o-z)

      dimension row(ix,jx,2),prw(ix,jx,2),vxw(ix,jx,2)
      dimension vyw(ix,jx,2),byw(ix,jx,2),bxw(ix,jx,2)
      dimension vzw(ix,jx,2),bzw(ix,jx,2)
      dimension fro(ix,jx),fee(ix,jx),frx(ix,jx),fry(ix,jx),fby(ix,jx)
...
c     this routine returns the numerical fluxes  
c     of 1st order acuracy, given left ans right physical value.
...
c----- amplitude;w's
      ...
      du6=byr-byl
      du7=bzr-bzl
      t1=betay*du6+betaz*du7
      t2=(prr-prl+(byave*du6+bzave*du7)*pi4i
     1  +(gm-2.0d0)*(bybar*du6+bzbar*du7)*pi4i)/(gm-1.0d0)
      t3=betaz*du6-betay*du7
      s1=du21
