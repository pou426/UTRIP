...
c======================================================================|
      ...
      dimension rodx(ix,jx)
      dimension rody(ix,jx)
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,vx,vxm,vy,vym,margin,x,y,ix,jx
     &    ,mf_params)

      call grdrdy(dx,xm,dxm,x,ix)
      call grdrdy(dy,ym,dym,y,jx)

      call ciprdy_a(ro,rodx,rody,dx,ix,dy,jx)

      call bnd(margin,ro,ix,jx)
      call cipbnd(margin,ro,rodx,rody,ix,jx)
...
c     data output
      ...
      write(mf_y) y
      mf_vx=13
      call dacdef2d(mf_vx,'vx.dac',6,ix,jx)
      write(mf_vx) vx
      mf_vxm=14
      call dacdef2d(mf_vxm,'vxm.dac',6,ix,jx)
      write(mf_vxm) vxm
      mf_vy=15
      call dacdef2d(mf_vy,'vy.dac',6,ix,jx)
      write(mf_vy) vy
      mf_vym=16
      call dacdef2d(mf_vym,'vym.dac',6,ix,jx)
      write(mf_vym) vym
...
c     obtain time spacing
         ...
         safety=0.4d0
         dtmin=1.d-10
         call cfl_a(dt,safety,dtmin,merr,vx,vy,dx,ix,dy,jx)
...
c     solve hydrodynamic equations
         ...
         call cip_a(ro,rodx,rody,dt,vx,vxm,vy,vym,dx,dxm,ix,dy,dym,jx)
