...
c======================================================================|
      ...
      parameter (margin=2)
      parameter (ix=128+2*margin,jx=256+2*margin)
...
c  file open
      ...
      call dacputparamc(mf_params,'comment','cans2d md_flare')
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,pr,vx,vy,vz,bx,by,bz,gm,rkap0
...
c     data output
      ...
      mf_et=13
      call dacdef2d(mf_et,'et.dac',6,ix,jx)
      write(mf_et) et
      mf_etm=14
      call dacdef2d(mf_etm,'etm.dac',6,ix,jx)
      write(mf_etm) etm
...
c     obtain time spacing
         ...
         safety=0.1d0
         dtmin=1.d-10
         call cfl_m3_e(dt,safety,dtmin,merr,gm,ro,pr,vx,vy,vz,bx,by,bz
     &                  ,et,dx,ix,dy,jx)
...
c                                                      hdmlw - start >>>
         qav=3.d0
         call mlw_m3_e(ro,pr,vx,vy,vz,bx,by,bz,az,dt,qav,gm
...
c                                                 conduction - start >>>
         call cndbicg_m3(ro,pr,mi,err,dt,gm,rkap0,bx,by,bz
     &                   ,margin,dx,dxm,ix,dy,dym,jx)
c                                                 conduction - end   <<<
        if (mod(ns,100).eq.0) write(6,925) ns,mi,err
925   format(' ns= ',i10,' mi= ',i5,' err= ',e14.6)
         call bnd(margin,ro,pr,vx,vy,vz,bx,by,bz,az,ix,jx)
c        floor=1.d-9
