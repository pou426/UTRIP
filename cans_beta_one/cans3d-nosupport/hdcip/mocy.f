c======================================================================|
      subroutine mocy(vycx,vycz,bycx,bycz,ro,vxm,bxm,vym,bym,vzm,bzm
...
c
c OUTPUTS
c    vyc(ix,jx,kx): [double] velocity 
c    byc(ix,jx,kx): [double] magnetic field
c
c INPUTS
...
c- Characteristic velocity
     ...
     &               +ro(i,j,k+1)+ro(i+1,j,k+1)))
        ...
        car(i,j,k) = 0.5*(vxm(i,j,k)+vxm(i,j,k+1))
     &             + 0.5*(bxm(i,j,k)+bxm(i,j,k+1))/sqrt(4.*pi*roh)
        cal(i,j,k) = 0.5*(vxm(i,j,k)+vxm(i,j,k+1))
     &             - 0.5*(bxm(i,j,k)+bxm(i,j,k+1))/sqrt(4.*pi*roh)
      enddo
      enddo
      enddo
...
c- van Leer interpolation
      ...
      call intpvl(vl,vr,vym,cal,car,dxm,dym,dzm,dt,ix,jx,kx,1)
      call intpvl(bl,br,bym,cal,car,dxm,dym,dzm,dt,ix,jx,kx,1)
...
c- MOC
      ...
      do i=1,ix-1
        ...
        vycx(i,j,k)=(vr(i,j,k)*ror4+vl(i,j,k)*rol4-br(i,j,k)+bl(i,j,k))
     &             /(ror4+rol4)
...
c- Characteristic velocity
     ...
     &               +ro(i,j,k+1)+ro(i+1,j,k+1)))
        ...
        car(i,j,k) = 0.5*(vzm(i,j,k)+vzm(i+1,j,k))
     &             + 0.5*(bzm(i,j,k)+bzm(i+1,j,k))/sqrt(4.*pi*roh)
        cal(i,j,k) = 0.5*(vzm(i,j,k)+vzm(i+1,j,k))
     &             - 0.5*(bzm(i,j,k)+bzm(i+1,j,k))/sqrt(4.*pi*roh)
      enddo
      enddo
      enddo
...
c- van Leer interpolation
      ...
      call intpvl(vl,vr,vym,cal,car,dxm,dym,dzm,dt,ix,jx,kx,3)
      call intpvl(bl,br,bym,cal,car,dxm,dym,dzm,dt,ix,jx,kx,3)
...
c- MOC
      ...
      do i=1,ix-1
        ...
        vycz(i,j,k)=(vr(i,j,k)*ror4+vl(i,j,k)*rol4-br(i,j,k)+bl(i,j,k))
     &             /(ror4+rol4)
