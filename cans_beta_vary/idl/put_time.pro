pro put_time,time,charsize=charsize,xposition=xp,yposition=yp,color=color,digit=digit

if (n_elements(digit) eq 0) then digit=4
...
if time lt 100. then $
time_st=strmid(strcompress(string(time),/remove_all),0,digit)
if time ge 100. then $
time_st=strmid(strcompress(string(time),/remove_all),0,digit+1)
