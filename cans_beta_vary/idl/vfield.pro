PRO VFIELD,U,V,X,Y, $
       ...
       Xstyle=xstyle,Ystyle=ystyle, Device = device, Normal = normal, $
...
if n_elements(iplot) eq 0 then iplot=i0skip+iskip*lindgen((((s(1)-i0skip)>0)/iskip)>1)
if n_elements(jplot) eq 0 then jplot=j0skip+jskip*lindgen((((s(2)-j0skip)>0)/jskip)>1)
...
;;;;;;;;; Frame ;;;;;;;;;;;;;;;
        if n_elements(position) eq 0 then begin
            ...
            plot,[x0,x1],[y1,y0],/nodata,$
              xstyle=xstyle,ystyle=ystyle, $
              title=title,xtitle=xtitle,ytitle=ytitle,$
              noerase=noerase, color=f_color, position=position,device=device,normal=normal,$
              xticks=xticks,yticks=yticks,xtickv=xtickv,ytickv=ytickv,$
              xtickname=xtickname,ytickname=ytickname,$
              xminor=xminor,yminor=yminor,zvalue=zvalue
        endelse
