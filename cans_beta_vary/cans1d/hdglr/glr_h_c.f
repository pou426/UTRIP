c======================================================================|
      subroutine glr_h_c(ro,pr,vx,x,dt,gm,dx,ix)
c======================================================================|
c
c NAME  glr_h_c
c
...
c         write(*,*)i,p1,p2,d1,d2,v1,v2,paa,vadt(i)/dt
      enddo
      
      do i=2,ix-1
         xeuler(i) = x(i)
         dxinit(i) = dx(i)
         dxlagc(i) = ((x(i)+0.5*dx(i))**igmp1
     &               -(x(i)-0.5*dx(i))**igmp1)*ro(i)/real(igmp1)
         dxlagi    = 1.d0/dxlagc(i)
         dxinii(i) = ro(i)/dxlagc(i)  !1.0d0/dx(i)
...
c         write(*,*)i,pa(i),vadt(i),dt!pa1,pa2,vadt1,vadt2
         ...
         xa1 = (x(i) - 0.5*dx(i))**igeom
         xa2 = (x(i) + 0.5*dx(i))**igeom

         volume = dx(i)*dxlagi + dxlagi*(xa2*vadt2 - xa1*vadt1)

         x(i)   = x(i) +0.5d0*(Vadt2 + Vadt1)
         vx(i)  = vx(i) -dt*dxlagi*(Pa2 - Pa1)*x(i)**igeom 
         dx(i)  = dx(i) + (xa2*Vadt2 - xa1*Vadt1)
         ro(i)  = 1.0d0/volume
         te(i)  = te(i) - dxlagi*(xa2*Pa2*Vadt2 - xa1*Pa1*Vadt1)
