...
c======================================================================|
      ...
      dimension ro(ix)
      dimension vx(ix),vy(ix),vz(ix),by(ix),bz(ix)
...
c  file open
      ...
      mf_vz=24
      call dacdef1s(mf_vz,'vz.dac',6,ix)
      mf_by=26
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,vx,vy,by,vz,bz,bx,bxm,cs2,margin,x,ix,mf_params)
      call pertub(vx,x,ix)

      call grdrdy(dx,xm,dxm,x,ix)
      call ciprdy_m3t(vxm,rodx,vxdxm,vydx,vzdx,ro,vx,vy,vz
     &       ,dx,dxm,ix)

      call bnd(margin,ro,vx,vy,by,vz,bz,ix)
      call cipbnd(margin,vxm,vy,vz,by,bz
...
c  read-data
      ...
      mfi_vz=74
      call dacopnr1s(mfi_vz,'in/vz.dac',mtype,ix0,nx0)
      mfi_by=76
...
c     obtain time spacing
         ...
         safety=0.2d0
         dtmin=1.d-10
         call cfl_m3t(dt,safety,dtmin,merr,cs2
...
c     solve hydrodynamic equations
         ...
         cvis=0.75d0
         call  cip_m3t(ro,vx,vy,vz,by,bz,vxm
     &    ,rodx,vxdxm,vydx,vzdx,bx,bxm,dt,cvis,cs2,dx,dxm,ix)
