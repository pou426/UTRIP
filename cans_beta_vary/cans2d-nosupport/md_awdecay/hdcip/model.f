...
c----------------------------------------------------------------------|
      csca2=0.1d0
      b_b0=0.9d0
      ampaw=sqrt(b_b0/2.d0)

      b0=sqrt(cs2/csca2*4*pi)
      ...
      do i=1,ix
         ...
         ro(i,j)  = 1.d0
         vs = 0.0d0
         vn = ampaw*b0/sqrt(4.d0*pi)*sin(phase)
         vt = ampaw*b0/sqrt(4.d0*pi)*cos(phase)
         vx(i,j)  = vs*cos(thini)-vn*sin(thini)
         vy(i,j)  = vs*sin(thini)+vn*cos(thini)
         vz(i,j)  = vt
         bs = b0
         bn  = -ampaw*b0*sin(phase)
         bt  = -ampaw*b0*cos(phase)
