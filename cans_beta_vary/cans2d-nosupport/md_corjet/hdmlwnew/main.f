...
c   setup numerical model (grid, initial conditions, etc.)
     ...
     &    ,mf_params)
      ...
      call etaanom(et,etm,bx,by,ro,dx,ix,dy,jx)
      ...
      safety=0.4d0
      dtmin=1.d-10
      call cfl_g(dt_gx,safety,dtmin,gx,dx,ix,jx,1,ix,merr)
      call cfl_g(dt_gy,safety,dtmin,gy,dy,ix,jx,2,jx,merr)
      dt_g=min(dt_gx,dt_gy)
...
c     obtain time spacing
      ...
      safety=0.4d0
      dtmin=1.d-10
      call cfl_m(dt_m,safety,dtmin,merr,gm,ro,pr,vx,vy,bx,by
     &      ,dx,ix,dy,jx)
      if (merr.ne.0) goto 9999
      call cfl_e(dt_e,safety,dtmin,et,dx,dy,ix,jx,merr)
      if (merr.ne.0) goto 9999
      dt=min(dt_m,dt_e,dt_g)
...
c     solve hydrodynamic equations
      ...
      call engine(ro,pr,vx,vy,bx,by,az,dt,gm
