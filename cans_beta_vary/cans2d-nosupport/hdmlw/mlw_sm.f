c======================================================================|
      subroutine mlw_sm(ro,pr,vx,vy,bx,by,az,dt,qav,gm
     &   ,dx,dxm,ix,dy,dym,jx)
c======================================================================|
c
c NAME  mlw_m
...
c----------------------------------------------------------------------|
     ...
     &         ,daz(ix,jx)
      ...
      dimension gl(ix,jx),de(ix,jx),pm(ix,jx),el(ix,jx)
      dimension glh(ix,jx),deh(ix,jx),pmh(ix,jx),elh(ix,jx)
c----------------------------------------------------------------------|
      pi = acos(-1.0d0)
      pi8i=1./pi/8.
      pi4i=1./pi/4.
...
c----------------------------------------------------------------------|
      ...
      do i=1,ix
         ...
         ee(i,j) = el(i,j)-pr(i,j)-de(i,j)+pm(i,j)
         rx(i,j) = el(i,j)*vx(i,j)+pi4i*(-ez(i,j)*by(i,j))
         ry(i,j) = el(i,j)*vy(i,j)+pi4i*( ez(i,j)*bx(i,j))
      enddo
      enddo
c----------------------------------------------------------------------|
...
c---  x-momentum ---
      do j=1,jx
      do i=1,ix       
         fx(i,j)= el(i,j)*vx(i,j)**2-pi4i*(bx(i,j)**2)+pr(i,j)+pm(i,j)
         fy(i,j)= el(i,j)*vx(i,j)*vy(i,j)-pi4i*(bx(i,j)*by(i,j))
      enddo
      enddo
...
c---  y-momentum ---
      do j=1,jx
      do i=1,ix       
         fx(i,j)= el(i,j)*vx(i,j)*vy(i,j)-pi4i*(bx(i,j)*by(i,j))
         fy(i,j)= el(i,j)*vy(i,j)**2-pi4i*(by(i,j)**2)+pr(i,j)+pm(i,j)
      enddo
      enddo
...
c---- solve nonlinear equations
     ...
     &     ,deh,eeh,rxh,ryh,bxh,byh,gm,mix,tolf,tolx
     &     ,ix,1,ix-1,jx,1,jx-1)
      ...
      do i=1,ix-1
        ...
        ef2 = ezh(i,j)**2
        bf2 = bxh(i,j)**2+byh(i,j)**2
        pmh(i,j)= pi8i*(bf2+ef2)
        enh = roh(i,j)+prh(i,j)/(gm-1)
        elh(i,j) = (enh+prh(i,j))*glh(i,j)**2
      enddo
      enddo
c----------------------------------------------------------------------|
...
c---  x-momentum ---
      do j=1,jx-1
      do i=1,ix-1
         fx(i,j)= elh(i,j)*vxh(i,j)**2-pi4i*(bxh(i,j)**2)
     &            +prh(i,j)+pmh(i,j)
         fy(i,j)= elh(i,j)*vxh(i,j)*vyh(i,j)-pi4i*(bxh(i,j)*byh(i,j))
      enddo
      enddo
...
c---  y-momentum ---
      do j=1,jx-1
      do i=1,ix-1
         fx(i,j)= elh(i,j)*vxh(i,j)*vyh(i,j)-pi4i*(bxh(i,j)*byh(i,j))
         fy(i,j)= elh(i,j)*vyh(i,j)**2-pi4i*(byh(i,j)**2)
     &            +prh(i,j)+pmh(i,j)
      enddo
      enddo
