...
c  file open
      ...
      call dacputparamc(mf_params,'comment','cans2d md_mhdcloud')
...
c----------------------------------------------------------------------|
c     initial calculation for selfgravity
c                                                      selfg - start >>>
      call selfginit(uod,nx)
      call selfgper(ro,uod,gx,gy,x,y,ng,nx)
c                                                      selfg - end   <<<
c----------------------------------------------------------------------|
c     ready
...
c     obtain time spacing
         ...
         safety=0.4d0
         dtmin=1.d-10
         call cfl_ht(dt,safety,dtmin,merr,cs2,vx,vy,dx,ix,dy,jx)
...
c     Jeans Condition
         ...
         rj=dx(1)/sqrt(2.*pi*pi/romax)
c        write(6,*) time,romax,rj
c----------------------------------------------------------------------|
c    solve gravity equation
c                                                      selfg - start >>>
      call selfgper(ro,uod,gx,gy,x,y,ng,nx)
      call ggtoggm(gx,gxm,gy,gym,ix,jx)
c                                                      selfg - end   <<<
c----------------------------------------------------------------------|
...
c                                                      hdmlw - start >>>
         qav=3.d0
         call mlw_mt_g(ro,vx,vy,bx,by,az,dt,qav,cs2,gx,gxm,gy,gym
     &                     ,dx,dxm,ix,dy,dym,jx)
c                                                      hdmlw - end   <<<
