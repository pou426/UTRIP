...
c  file open
      ...
      call dacputparamc(mf_params,'comment','cans2d md_disknwi')
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,pr,vx,vy,gm,gx,gxm,gy,gym,margin,x,ix,y,jx
     &    ,mf_params)
      call saveini(roi,ro,ix,jx)
      call saveini(pri,pr,ix,jx)
      call saveini(vyi,vy,ix,jx)
      call bnd(margin,ro,pr,vx,vy,ix,jx,roi,pri,vyi)
      floor=1.d-9
...
c     obtain time spacing
         ...
         safety=0.4d0
         dtmin=1.d-10
         call cfl_h_d(dt,safety,dtmin,merr,gm,ro,pr,vx,vy,x,dx,ix,dy,jx)
...
c                                                      hdmlw - start >>>
         qav=3.d0
         call mlw_h_dg(ro,pr,vx,vy,dt,qav,gm,gx,gxm,gy,gym
