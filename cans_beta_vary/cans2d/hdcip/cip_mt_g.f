c======================================================================|
      subroutine cip_mt(ro,vx,vxm,vy,vym
     &             ,rodx,rody,vxdxm,vxdym,vydxm,vydym
     &             ,bx,bxm,by,bym,az
     &             ,dt,cvis,cs2,gxm,gym,dx,dxm,ix,dy,dym,jx)
c======================================================================|
c
c NAME  cip_m
c
...
c----------------------------------------------------------------------|
      ...
      dimension rodx(ix,jx),rody(ix,jx)
      dimension vxdxm(ix,jx),vxdym(ix,jx),vydxm(ix,jx),vydym(ix,jx)
      dimension roh(ix,jx),vxmh(ix,jx),vymh(ix,jx)
      dimension rodxh(ix,jx),rodyh(ix,jx)
...
c--- ### important ### Calculate magnetic stress term Using MOC method
      ...
      do i=2,ix-1
        vymh(i,j)=vymh(i,j)
     &    +1./(4.*pi*ro(i,j))
     &      *(bxm(i,j)+bxm(i-1,j)+bxm(i,j+1)+bxm(i-1,j+1))/4
     &      *(byc(i,j)-byc(i-1,j))/dx(i)*dt
      enddo
      enddo
      do j=2,jx-1
      do i=1,ix-1
        vxmh(i,j)=vxmh(i,j)
     &    +1./(4.*pi*ro(i,j))
     &      *(bym(i,j)+bym(i+1,j)+bym(i,j-1)+bym(i+1,j-1))/4
     &      *(bxc(i,j)-bxc(i,j-1))/dy(j)*dt
      enddo
      enddo
...
c--- ### important ### velocity must be advanced first.
     ...
     &    *(pr(i+1,j)-pr(i,j)+pi4i*(by(i+1,j)**2-by(i,j)**2)/2)
     &       +dt*gxm(i,j)
      enddo
      enddo
      do j=1,jx-1
     ...
     &    *(pr(i,j+1)-pr(i,j)+pi4i*(bx(i,j+1)**2-bx(i,j)**2)/2)
     &       +dt*gym(i,j)
      enddo
      enddo
