c======================================================================|
      subroutine cip_m_g(ro,pr,vx,vxm,vy,vym,te
...
c--- ### important ### velocity must be advanced first.
     ...
     &       +dt*gym(i,j)
c     if (i.eq.150.and.j.eq.3) then
c       write(6,*) -2*dt/dym(j)/(ro(i,j)+ro(i,j+1))
c    &    *(pr(i,j+1)-pr(i,j)+pi4i*(bx(i,j+1)**2-bx(i,j)**2)/2)
c    &       ,dt*gym(i,j)
c     endif
      enddo
      enddo
