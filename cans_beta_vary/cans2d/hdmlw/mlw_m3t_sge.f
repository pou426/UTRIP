c======================================================================|
      subroutine mlw_m3t_sge(ro,vx,vy,vz,bx,by,bz,az,dt,qav,cs2
     &      ,gx,gxm,gy,gym,et,etm,x,xm,dx,dxm,ix,y,ym,dy,dym,jx)
c======================================================================|
c
c NAME  mlw_m3t_sge
c
...
c----------------------------------------------------------------------|
      ...
      call bbtocx_s(cx,bz,dy,x,y,ix,jx)
      call bbtocy_s(cy,bz,dx,x,y,ix,jx)
      call bbtocz_s(cz,bx,by,dx,dy,x,ix,jx)

      do j=1,jx
      do i=1,ix
         ex(i,j) = -vy(i,j)*bz(i,j)+vz(i,j)*by(i,j)+et(i,j)*cx(i,j)
         ey(i,j) = -vz(i,j)*bx(i,j)+vx(i,j)*bz(i,j)+et(i,j)*cy(i,j)
         ez(i,j) = -vx(i,j)*by(i,j)+vy(i,j)*bx(i,j)+et(i,j)*cz(i,j)
      enddo
      enddo
      do j=1,jx
      do i=1,ix
...
c---  x-momentum ---
     ...
     &          + (ro(i,j)*(vy(i,j)**2+vz(i,j)**2)+2*pr(i,j)
...
c----------------------------------------------------------------------|
      ...
      call bbtocx_s(cxh,bzh,dym,xm,ym,ix,jx)
      call bbtocy_s(cyh,bzh,dxm,xm,ym,ix,jx)
      call bbtocz_s(czh,bxh,byh,dxm,dym,xm,ix,jx)

      do j=1,jx
      do i=1,ix
        exh(i,j)=-vyh(i,j)*bzh(i,j)+vzh(i,j)*byh(i,j)+etm(i,j)*cxh(i,j)
        eyh(i,j)=-vzh(i,j)*bxh(i,j)+vxh(i,j)*bzh(i,j)+etm(i,j)*cyh(i,j)
        ezh(i,j)=-vxh(i,j)*byh(i,j)+vyh(i,j)*bxh(i,j)+etm(i,j)*czh(i,j)
      enddo
      enddo
c----------------------------------------------------------------------|
...
c---  x-momentum ---
     ...
     &          + (roh(i,j)*(vyh(i,j)**2+vzh(i,j)**2)+2*prh(i,j)
