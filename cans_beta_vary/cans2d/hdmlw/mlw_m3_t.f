c======================================================================|
      subroutine mlw_m3_t(ro,pr,vx,vy,vz,bx,by,bz,ay,dt,qav,gm
     &                   ,omega0,qt0,x,xm,dx,dxm,ix,dz,dzm,jx)
c======================================================================|
c
c NAME  mlw_m3_t
c
...
c    
c    omega0: [double] rotation angular velocity of the reference frame
c    qt0: [double] tidal force parameter [=-d(ln Omega)/d(ln R)]
c         qt0=3/2 for Kepler, qt0=1 for uniform speed like Galaxy
...
c---  energy ---
      ...
      do i=1,ix       
         ...
         ss(i,j)= 2.0d0*qt0*omega0**2*x(i)*ro(i,j)*vx(i,j)
      enddo
      enddo
...
c---  x-momentum ---
     ...
     &          +pi8i*(by(i,j)**2+bz(i,j)**2-bx(i,j)**2)
         ...
         ss(i,j)= 2.0d0*qt0*omega0**2*x(i)*ro(i,j)
     &           +2.0d0*omega0*ro(i,j)*vy(i,j)
      enddo
      enddo
...
c---  y-momentum ---
      ...
      do i=1,ix       
         ...
         ss(i,j)= -2.0d0*omega0*ro(i,j)*vx(i,j)
      enddo
      enddo
...
c---  energy     ---
     ...
     &            +(byh(i,j)*exh(i,j)-bxh(i,j)*eyh(i,j))*pi4i
         ss(i,j)= 2.0d0*qt0*omega0**2*xm(i)*roh(i,j)*vxh(i,j)
      enddo
      enddo
...
c---  x-momentum ---
     ...
     &          +pi8i*(byh(i,j)**2+bzh(i,j)**2-bxh(i,j)**2)
         ...
         ss(i,j)= 2.0d0*qt0*omega0**2*xm(i)*roh(i,j)
     &           +2.0d0*omega0*roh(i,j)*vyh(i,j)
      enddo
      enddo
...
c---  y-momentum ---
      ...
      do i=1,ix       
         ...
         ss(i,j)= -2.0d0*omega0*roh(i,j)*vxh(i,j)
      enddo
      enddo
