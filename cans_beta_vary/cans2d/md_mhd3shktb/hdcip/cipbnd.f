c======================================================================|
      subroutine cipbnd(margin,ro,te,vxm,vym,vz,bxm,bym,bz
     &       ,rodx,tedx,vxdxm,vydxm,vzdx
     &       ,rody,tedy,vxdym,vydym,vzdy
     &       ,ix,jx)
c======================================================================|
c     apply boundary condition 
c----------------------------------------------------------------------|      
      implicit double precision (a-h,o-z)
      dimension ro(ix,jx),te(ix,jx),vxm(ix,jx),vym(ix,jx)
      dimension rodx(ix,jx),tedx(ix,jx),vxdxm(ix,jx),vydxm(ix,jx)
      dimension rody(ix,jx),tedy(ix,jx),vxdym(ix,jx),vydym(ix,jx)
      dimension bx(ix,jx),by(ix,jx),az(ix,jx)
      dimension vz(ix,jx),vzdx(ix,jx),vzdy(ix,jx)
      dimension bz(ix,jx)
c----------------------------------------------------------------------|      
      call bdfrex(0,margin,ro,ix,jx)
      call bdcnsx(0,margin,rodx,0.d0,ix,jx)
      call bdfrex(0,margin,rody,ix,jx)
      call bdfrex(0,margin,te,ix,jx)
      call bdcnsx(0,margin,tedx,0.d0,ix,jx)
      call bdfrex(0,margin,tedy,ix,jx)
      call bdfrex(0,margin-1,vxm,ix,jx)
      call bdcnsx(0,margin-1,vxdxm,0.d0,ix,jx)
      call bdfrex(0,margin-1,vxdym,ix,jx)
      call bdfrex(0,margin,vym,ix,jx)
      call bdcnsx(0,margin,vydxm,0.d0,ix,jx)
      call bdfrex(0,margin,vydym,ix,jx)
      call bdfrex(0,margin,vz,ix,jx)
      call bdcnsx(0,margin,vzdx,0.d0,ix,jx)
      call bdfrex(0,margin,vzdy,ix,jx)
      call bdfrex(0,margin-1,bxm,ix,jx)
      call bdfrex(0,margin,bym,ix,jx)
      call bdfrex(0,margin,bz,ix,jx)

      call bdfrex(1,margin,ro,ix,jx)
      call bdcnsx(1,margin,rodx,0.d0,ix,jx)
      call bdfrex(1,margin,rody,ix,jx)
      call bdfrex(1,margin,te,ix,jx)
      call bdcnsx(1,margin,tedx,0.d0,ix,jx)
      call bdfrex(1,margin,tedy,ix,jx)
      call bdfrex(1,margin,vxm,ix,jx)
      call bdcnsx(1,margin,vxdxm,0.d0,ix,jx)
      call bdfrex(1,margin,vxdym,ix,jx)
      call bdfrex(1,margin,vym,ix,jx)
      call bdcnsx(1,margin,vydxm,0.d0,ix,jx)
      call bdfrex(1,margin,vydym,ix,jx)
      call bdfrex(1,margin,vz,ix,jx)
      call bdcnsx(1,margin,vzdx,0.d0,ix,jx)
      call bdfrex(1,margin,vzdy,ix,jx)
      call bdfrex(1,margin,bxm,ix,jx)
      call bdfrex(1,margin,bym,ix,jx)
      call bdfrex(1,margin,bz,ix,jx)

      call bdfrey(0,margin,ro,ix,jx)
      call bdfrey(0,margin,rodx,ix,jx)
      call bdcnsy(0,margin,rody,0.d0,ix,jx)
      call bdfrey(0,margin,te,ix,jx)
      call bdfrey(0,margin,tedx,ix,jx)
      call bdcnsy(0,margin,tedy,0.d0,ix,jx)
      call bdfrey(0,margin,vxm,ix,jx)
      call bdfrey(0,margin,vxdxm,ix,jx)
      call bdcnsy(0,margin,vxdym,0.d0,ix,jx)
      call bdfrey(0,margin-1,vym,ix,jx)
      call bdfrey(0,margin-1,vydxm,ix,jx)
      call bdcnsy(0,margin-1,vydym,0.d0,ix,jx)
      call bdfrey(0,margin,vz,ix,jx)
      call bdfrey(0,margin,vzdx,ix,jx)
      call bdcnsy(0,margin,vzdy,0.d0,ix,jx)
      call bdfrey(0,margin,bxm,ix,jx)
      call bdfrey(0,margin-1,bym,ix,jx)
      call bdfrey(0,margin,bz,ix,jx)

      call bdfrey(1,margin,ro,ix,jx)
      call bdfrey(1,margin,rodx,ix,jx)
      call bdcnsy(1,margin,rody,0.d0,ix,jx)
      call bdfrey(1,margin,te,ix,jx)
      call bdfrey(1,margin,tedx,ix,jx)
      call bdcnsy(1,margin,tedy,0.d0,ix,jx)
      call bdfrey(1,margin,vxm,ix,jx)
      call bdfrey(1,margin,vxdxm,ix,jx)
      call bdcnsy(1,margin,vxdym,0.d0,ix,jx)
      call bdfrey(1,margin,vym,ix,jx)
      call bdfrey(1,margin,vydxm,ix,jx)
      call bdcnsy(1,margin,vydym,0.d0,ix,jx)
      call bdfrey(1,margin,vz,ix,jx)
      call bdfrey(1,margin,vzdx,ix,jx)
      call bdcnsy(1,margin,vzdy,0.d0,ix,jx)
      call bdfrey(1,margin,bxm,ix,jx)
      call bdfrey(1,margin,bym,ix,jx)
      call bdfrey(1,margin,bz,ix,jx)

      return
      end
