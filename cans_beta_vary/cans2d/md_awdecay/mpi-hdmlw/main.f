...
c======================================================================|
      ...
      parameter (margin=1)
      parameter (ix=128/ipex+2*margin)
      parameter (jx=128/jpex+2*margin)
...
c   setup numerical model (grid, initial conditions, etc.)
      ...
      call model(ro,vx,vy,vz,bx,by,bz,cs2,thini
     &        ,margin,x,ix,y,jx,mf_params
     &           ,igx,jgx,ipe,jpe)
      call pertub(thini,vx,vy,x,ix,y,jx)
      mper=(/1,1/)
      call grdrdy(dx,xm,dxm,x,ix)
      call grdrdy(dy,ym,dym,y,jx)
      call p_bbtoaa(az,bx,by,dxm,dym,margin,ix,jx
     &           ,igx,jgx,ipe,jpe,ipex,jpex)
c     call exc_8(margin,ro,vx,vy,vz,bx,by,bz,az,ix,jx
c    &           ,ipe,jpe,ipex,jpex,mper)
c     call bnd(margin,ro,vx,vy,vz,bx,by,bz,az,ix,jx
c    &           ,ipe,jpe,ipex,jpex)
c     floor=1.d-9
...
c     obtain time spacing
         ...
         safety=0.4d0
         dtmin=1.d-10
         call cfl_m3t(dt,safety,dtmin,merr,cs2,ro,vx,vy,vz,bx,by,bz
     &     ,dx,ix,dy,jx)
         dtarr=(/dt,dble(-merr)/)
         dtarrg=(/0.d0,0.d0/)
...
c     solve hydrodynamic equations
         ...
         qav=3.d0
         call mlw_m3t(ro,vx,vy,vz,bx,by,bz,az
     &                 ,dt,qav,cs2,dx,dxm,ix,dy,dym,jx)

         call exc_8(margin,ro,vx,vy,vz,bx,by,bz,az,ix,jx
     &           ,ipe,jpe,ipex,jpex,mper)
         call bnd(margin,ro,vx,vy,vz,bx,by,bz,az,ix,jx
     &           ,ipe,jpe,ipex,jpex)
c        floor=1.d-9
c        call chkdav(n_floor,ro,vx,vy,floor,ix,jx)
