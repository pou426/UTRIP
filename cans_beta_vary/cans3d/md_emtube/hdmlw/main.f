...
c======================================================================|
      ...
      parameter (margin=1)
      parameter (ix= 64+2*margin,jx=64+2*margin,kx=128+2*margin)
...
c   setup numerical model (grid, initial conditions, etc.)
     ...
     &           ,mf_params,igx,jgx,kgx,ipe,jpe,kpe)
      call pertub(ro,x,ix,y,jx,z,kx,rtube,ytube,ztube)
      ...
      do j=1,jx
      do i=1,ix
        droz(i,j,k)=ro(i,j,kx-margin+k)-ro(i,j,kx-margin)
        dprz(i,j,k)=pr(i,j,kx-margin+k)-pr(i,j,kx-margin)
      enddo
      enddo
      enddo
c     call bnd(margin,ro,pr,vx,vy,vz,bx,by,bz,droz,dprz,ix,jx,kx)
c     floor=1.d-9
...
c     solve hydrodynamic equations
         ...
         qav=3.d0
         call mlw_m_g(ro,pr,vx,vy,vz,bx,by,bz,dt,qav,gm
     &            ,gx,gxm,gy,gym,gz,gzm,dx,dxm,ix,dy,dym,jx,dz,dzm,kx)

      call bnd(margin,ro,pr,vx,vy,vz,bx,by,bz,droz,dprz,ix,jx,kx)
c        floor=1.d-9
c        call chkdav(n_floor,ro,vx,vy,vz,floor,ix,jx,kx)
c        call chkdav(n_floor,pr,vx,vy,vz,floor,ix,jx,kx)
